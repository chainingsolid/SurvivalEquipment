...
public abstract class SurvivalUtilsGui extends GuiContainer {
	...
	public SurvivalUtilsGui(SurvivalEquipmentContainer par1Container) {
		super(par1Container);
		...
		try{
			for(int i = 0; i < contianer.inventorySlots.size();i++){
				...
				if((s instanceof ArmorSlot)){
					this.drawArmorSlot(s.xDisplayPosition, s.yDisplayPosition);
				}else if(s instanceof SlotButton){
	...
	protected void drawArmorSlot(int x,int y){
		Minecraft.getMinecraft().renderEngine.bindTexture(TEXTURE);
