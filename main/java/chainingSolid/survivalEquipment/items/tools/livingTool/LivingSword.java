...
public class LivingSword extends ItemSword implements LoadableItem, ILivingEquipment{
	
	public final String DAMADGE = "damadge",LEVEL = "level",CAN_REPIAR = "can_repair";
	public final int STARTING_DAMADGE = 10;
	
	public LivingSword(ToolMaterial m) {
		super(m);
		this.setNoRepair();
	}
	...
	public float getDigSpeed(ItemStack stack, Block block, int meta){
		...
		if(ForgeHooks.isToolEffective(stack, block, meta)){
			return (stack.getTagCompound().getInteger(LEVEL)+1)*.5F;
		}else{
			return 1;
		}
	}
